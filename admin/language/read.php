...
function nv_admin_read_lang($dirlang, $module, $admin_file = 1)
{
    ...
    if ($include_lang != '' and file_exists($include_lang)) {
        ...
        if (empty($idfile)) {
            ...
            $lang_translator_save['author'] = isset($lang_translator['author']) ? trim($lang_translator['author']) : 'Tập Đoàn TMS Holdings <contact@tms.vn>';
            $lang_translator_save['createdate'] = isset($lang_translator['createdate']) ? trim($lang_translator['createdate']) : date('d/m/Y, H:i');
            $lang_translator_save['copyright'] = isset($lang_translator['copyright']) ? trim($lang_translator['copyright']) : 'Copyright (C) ' . date('Y') . ' Tập Đoàn TMS Holdings. All rights reserved';
